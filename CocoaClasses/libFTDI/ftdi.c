...
int ftdi_usb_open_string(struct ftdi_context *ftdi, const char* description)
{
    ...
    else if (description[0] == 'i' || description[0] == 's')
    {
        ...
        return ftdi_usb_open_desc_index(ftdi, (unsigned int)vendor, (int)product, NULL, serial, (unsigned int)index);
    }
    else
    {
        ftdi_error_return(-11, "illegal description format");
...
int ftdi_write_data(struct ftdi_context *ftdi, unsigned char *buf, int size)
{
    ...
    while (offset < size)
    {
        ...
        ret = usb_bulk_write(ftdi->usb_dev, ftdi->in_ep, (const char *)buf+offset, write_size, ftdi->usb_write_timeout);
        if (ret < 0)

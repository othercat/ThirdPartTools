...
int simple_2xx(void)
{
	...
	for(i = 0; ( (i <MAX_DEVICES) && (i < iNumDevs) ) ; i++) {
		...
		if((ftStatus = FT_OpenEx(cBufLD[i], FT_OPEN_BY_SERIAL_NUMBER, &ftHandle_simple[i])) != FT_OK){
		 	...
		 	*/
            printf("Error FT_OpenEx(%u), device : %d\n", ftStatus, i);
			return 1;
		}
		...
		if((ftStatus = FT_SetBaudRate(ftHandle_simple[i], 115200)) != FT_OK) {
            ...
            printf("Error FT_SetBaudRate(%u), cBufLD[i] = %s\n", ftStatus, cBufLD[i]);
            break;
		}
		for(j = 0; j < BUF_SIZE; j++) {
		...
		if(ftStatus == FT_OK) {
			...
			printf("Error FT_GetQueueStatus(%u)\n", ftStatus);	
		}
	}
